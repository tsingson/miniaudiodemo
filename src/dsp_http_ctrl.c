...
static void send_http_response_parts(int clientFd,
                                     ...
                                     size_t partCount)
{
    ...
    for (size_t i = 0; i < partCount; ++i)
    {
        ...
        (void)send_all(clientFd, part, strlen(part));
    }
}

static const char* kWebUiHeadOpen = "<!doctype html><html><head><meta charset='utf-8'><title>EQ + Spectrum</title>";
static const char* kWebUiCss =
    ...
    "@media(max-width:900px){.dyn{grid-template-columns:repeat(2,minmax(0,1fr));}.sliderGroupGrid{grid-template-columns:repeat(2,minmax(0,1fr));}}</style>";
static const char* kWebUiBodyOpen =
    "</head><body><div class='wrap'><h1>实时频谱 + 多段EQ + 多段压缩</h1><div class='sub'>拖拽橙色控制点或下方滑块，实时改变播放 EQ</div>"
    "<div class='panel'><canvas id='c' width='1000' height='400'></canvas><div id='dyn' class='dyn'></div><div id='sliders' class='sliders'></div></div></div><script>";
static const char* kWebUiTail = "</script></body></html>";
...
static void handle_http_request(http_server_state* http, int clientFd)
{
    ...
    if (strncmp(req, "GET /eq", 7) == 0)
    {
        ...
        pluginMask = play_dsp_get_plugin_mask(&http->app->dsp);
        rmsLogEnabled = http->app->rmsLogEnabled ? 1 : 0;
        sampleRate = http->app->dsp.sampleRate;
        pthread_mutex_unlock(&http->app->dspMutex);
    ...
    if (strncmp(req, "POST /eq", 8) == 0)
    {
        ...
        hasRmsLogEnabled = parse_float_value_from_json(bodyStart, "\"rmsLogEnabled\"", &rmsLogEnabledValue);
        ...
        if (gainCount <= 0 && qCount <= 0 && !hasAnyDynamic && !hasPhasePluginEnabled && !hasBypassEnabled &&
            ...
            !hasLinearFirTaps && !hasRmsLogEnabled && !hasResetAll)
        {
        ...
        if (hasRmsLogEnabled)
        {
            http->app->rmsLogEnabled = (rmsLogEnabledValue >= 0.5f) ? 1u : 0u;
            http->app->rmsLogFrameAccum = 0u;
            http->app->rmsLogSumSq[0] = 0.0;
            http->app->rmsLogSumSq[1] = 0.0;
        }
    ...
    {
        const char* pageParts[] = {
            kWebUiHeadOpen,
            kWebUiCss,
            kWebUiBodyOpen,
            /* JS 组件1: 全局状态、频率坐标与基础数学工具函数。 */
            (
            "const c=document.getElementById('c');const g=c.getContext('2d');const slidersEl=document.getElementById('sliders');"
            ...
            "let specPre=[];let specPost=[];let phaseDelta=[];let groupDelay=[];let phaseEnabled=false;let bypassEnabled=false;let rmsLogEnabled=false;let lowCrossfeedEnabled=false;let lowCrossfeedPosition=0;let freqs=[];let gains=[];let qs=[];let modes=[];let dynReduction=[];let minGain=-12,maxGain=12,minQ=.3,maxQ=4,sampleRate=48000;"
            ...
            /* JS 组件2: 主图渲染、参数上报节流与顶部/分页 UI 同步。 */
            (
            ...
            /* JS 组件3: EQ/MB 动态控件生成与压缩曲线编辑区域绘制。 */
            (
