 ...
 *
 * VENDORED COPY - see src/uac2/README.md before editing.
 *
 * Base: zephyrproject-rtos/zephyr v4.4.2 drivers/usb/udc/udc_stm32.c (this is
 * the exact revision pinned by ZEPHYR_BASE for this project).
 * Patch applied on top: unmerged PR zephyrproject-rtos/zephyr#113622
 * ("drivers: usb: udc: stm32: recover isochronous OUT on incomplete
 * transfer"), adapted from `stm32_pcd_handle_t`/`stm32_status_t` (main-only
 * HAL2 abstraction) back to v4.4.2's plain `PCD_HandleTypeDef`/
 * `HAL_StatusTypeDef` types. Adds HAL_PCD_ISOOUTIncompleteCallback() /
 * HAL_PCD_ISOINIncompleteCallback() so an isochronous OUT endpoint (UAC2
 * playback) is re-armed after a missed (micro)frame instead of being
 * permanently disabled by the OTG core.
 *
 * Delete this vendored copy and switch back to the in-tree
 * drivers/usb/udc/udc_stm32.c (re-enable CONFIG_UDC_STM32) once #113622 (or
 * an equivalent fix) lands in the Zephyr revision this project builds
 * against.
 */
...
void HAL_PCD_ISOOUTIncompleteCallback(PCD_HandleTypeDef *hpcd, uint8_t epnum)
{
	 ...
	 */
	status = HAL_PCD_EP_Receive(&priv->pcd, ep_cfg->addr, net_buf_tail(buf),
