...
- src/dspeq/play_dsp_common.h
- src/dspeq/play_dsp_common.c
- src/dspeq/play_pipeline.h / src/dspeq/play_pipeline.c
- src/dspeq/play_pipeline_eq_stages.h / src/dspeq/play_pipeline_eq_stages.c
- src/dspeq/play_eq_linear.* / src/dspeq/play_eq_dynamic.* / src/dspeq/play_eq_normal.*
- src/dspeq/play_multiband_dynamics.*
...
定义位置：src/dspeq/play_dsp_common.h
...
### 4.1 src/dspeq/play_eq_linear.*

- 线性 FIR 配置与重建
- 样本处理函数与独立 ctx API

### 4.2 src/dspeq/play_eq_dynamic.*

- 动态包络更新
- 动态衰减计算
- 参数结构体（可独立初始化/设置/读取）

### 4.3 src/dspeq/play_eq_normal.*
...
### 4.4 src/dspeq/play_multiband_dynamics.*

- MB 参数初始化与钳制
- amount/applied/bands 拷贝接口
- 单样本 MB 处理核心

### 4.5 src/dspeq/play_pipeline.*

- pipeline 容器与 stage 调度
- stage add/enable/bypass/run

### 4.6 src/dspeq/play_pipeline_eq_stages.*
