...
- `CMakeLists.txt`：仅在 board-specific UAC2 分支追加 `src/uac2/udc_stm32.c` 到 `target_sources`，并把 `${ZEPHYR_BASE}/drivers/usb/udc` 加入 include 路径（给私有的 `"udc_common.h"` 用；`stm32_usb_common.h` 已经是 Zephyr 全局 include，不用额外加）。
