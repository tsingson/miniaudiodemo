...
Usage: ./r.sh <build|flash|monitor|all|build-demo|flash-demo|all-demo|build-demo-prod|flash-demo-prod|build-prod|flash-prod|build-pcm|flash-pcm|all-pcm|build-pcm-prod|flash-pcm-prod|build-pcm5102a-test|flash-pcm5102a-test|build-uac2-macos|run-uac2-macos|build-uac2-stm32|flash-uac2-stm32|build-uac2-implicit|flash-uac2-implicit|build-uac2-null|flash-uac2-null|build-uac2-null-implicit|flash-uac2-null-implicit|build-uac2-stm32-prod|flash-uac2-stm32-prod|build-uac2-h7b0|flash-uac2-h7b0|clean>
  ...
  build-uac2-h7b0  Build the FK7B0M1 STM32H7B0 UAC2 receiver + PCM5102A
  flash-uac2-h7b0  Flash the FK7B0M1 STM32H7B0 UAC2 receiver
...
build_uac2_stm32_app() {
  "$WEST" build -b "$BOARD" . --build-dir build-zephyr-f401rct6-uac2 --pristine -- -DEXTRA_CONF_FILE=prj_uac2.conf -DDTC_OVERLAY_FILE=boards/stm32f401rct6_uac2.overlay -DMINIAUDIO_PCM5102_STM32F401_UAC2_MAIN=ON -DMINIAUDIO_UAC2_PID=$(uac2_pid_arg)
}

build_uac2_null_app() {
  "$WEST" build -b "$BOARD" . --build-dir build-zephyr-f401rct6-uac2-null --pristine -- -DEXTRA_CONF_FILE=prj_uac2.conf -DDTC_OVERLAY_FILE=boards/stm32f401rct6_uac2.overlay -DMINIAUDIO_PCM5102_STM32F401_UAC2_MAIN=ON -DMINIAUDIO_UAC2_NULL_SINK=ON -DMINIAUDIO_UAC2_PID=$(uac2_pid_arg)
}

build_uac2_null_implicit_app() {
  "$WEST" build -b "$BOARD" . --build-dir build-zephyr-f401rct6-uac2-null-implicit --pristine -- -DEXTRA_CONF_FILE=prj_uac2.conf -DDTC_OVERLAY_FILE=boards/stm32f401rct6_uac2_implicit.overlay -DMINIAUDIO_PCM5102_STM32F401_UAC2_MAIN=ON -DMINIAUDIO_UAC2_NULL_SINK=ON -DMINIAUDIO_UAC2_PID=$(uac2_pid_arg)
}
...
build_uac2_implicit_app() {
  "$WEST" build -b "$BOARD" . --build-dir build-zephyr-f401rct6-uac2-implicit --pristine -- -DEXTRA_CONF_FILE=prj_uac2.conf -DDTC_OVERLAY_FILE=boards/stm32f401rct6_uac2_implicit.overlay -DMINIAUDIO_PCM5102_STM32F401_UAC2_MAIN=ON -DMINIAUDIO_UAC2_PID=$(uac2_pid_arg)
}

build_uac2_h7b0_app() {
  "$WEST" build -b mini_stm32h7b0 . --build-dir "$BUILD_DIR_H7B0_UAC2" --pristine -- -DEXTRA_CONF_FILE=prj_uac2.conf -DDTC_OVERLAY_FILE=boards/stm32h7b0vbt6_uac2.overlay -DMINIAUDIO_PCM5102_STM32H7B0_UAC2_MAIN=ON -DMINIAUDIO_UAC2_PID=$(uac2_pid_arg)
}

build_uac2_stm32_prod_app() {
  "$WEST" build -b "$BOARD" . --build-dir build-zephyr-f401rct6-uac2-prod --pristine -- -DCONF_FILE=prj_prod.conf -DEXTRA_CONF_FILE=prj_uac2.conf -DMINIAUDIO_PCM5102_STM32F401_UAC2_MAIN=ON
}

flash_app() {
  local build_dir="${1:-$BUILD_DIR}"
  local openocd_bin="$(command -v openocd || true)"

  if [[ -n "$openocd_bin" ]]; then
    echo "Flashing with Homebrew OpenOCD: $openocd_bin"

    echo "OpenOCD attempt 1: default settings"
    if PATH="$(dirname "$openocd_bin"):$PATH" "$WEST" flash -d "$build_dir" -r openocd; then
      return
    fi

    echo "OpenOCD attempt 2: connect-under-reset + slower SWD (1000 kHz)"
    if PATH="$(dirname "$openocd_bin"):$PATH" "$WEST" flash -d "$build_dir" -r openocd \
    ...
    echo "Tip: connect a supported SWD probe, keep NRST pressed, and release it when flashing starts." >&2
    exit 1
  fi
