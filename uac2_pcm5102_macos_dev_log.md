...
- **速率匹配协商**：走的是显式反馈端点（`feedback_cb`），时钟源没设 `sof-synchronized`，配置是自洽的（但见 3.9，"配置自洽"不等于"主机真的会用它"）。

### 3.9 内存重新分配踩了一次死锁坑：`PCM_PRIME_BLOCKS` 不能超过驱动自己的队列深度
...
### 3.10 加了直接可观测的 slab 占用诊断，证实残余卡顿确实是真实的时钟漂移
...
### 3.11 反馈机制本身其实没用上：主机在协商之后就不再轮询了
...
### 3.12 隐式反馈实验 + "换用自己的 macOS 发送端能不能帮上忙"的分析
...
`main_uac2_srv.c` 只是换了"喂给 CoreAudio 什么内容、怎么处理"，并没有绕开 CoreAudio 和苹果自带的 USB Audio 驱动——而 3.11 已经证实，卡顿根因正是这个黑盒驱动在协商后不再轮询我们的反馈端点，这是传输层行为，跟"用什么播放器/发送端"无关。
...
- 新增可切换的隐式反馈构建变体（`build-uac2-implicit`），作为对照实验，效果待实测。
...
1. **反馈机制**：显式反馈（explicit feedback）路线已确认在当前 macOS 行为下走不通。可选方向：
   - 隐式反馈对照实验（`build-uac2-implicit`）：待实测，效果未知。
   - **`main_uac2_srv.c` 静态速率补偿**（方向 A，见 3.12）：改动小、见效快，建议优先尝试。
   - **`main_uac2_srv.c` + USART1 应用层旁路动态反馈**（方向 B，见 3.12）：真正自适应的长期方案，工作量较大。
